...
- **Cloud config** (under "Advanced"): paste the entire contents of `cloud-init.yaml` from this repo. The VPS will auto-install Docker + Caddy + UFW + fail2ban + swap + clone the repo on first boot (~2 minutes), so you can skip most of Phase D.1.
- **Name**: `brein`.
- **Create & Buy now**.
...
The manifest is already in the repo at `static/manifest.webmanifest`. You need to provide three icon PNGs:
